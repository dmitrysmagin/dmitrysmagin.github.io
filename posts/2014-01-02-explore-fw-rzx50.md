...
и документации.
К счастью, имеется [зеркало](ftp://grids.be/mirror/ftp.ingenic.cn/), где есть всё, что Ingenic когда-либо выкладывала в общий доступ, включая руководство по программированию SoC от JZ4720 до JZ4760 включительно.
...
Это вылилось в частности в то, что вышли всего две версии ядра 2.6.24.3 и 2.6.31.3 и с тех пор не обновлялись вообще. В главную ветку ядра код Ingenic не был принят.
...
были внесены следующие изменения: теперь корневая файловая система rootfs располагается на своём собственном разделе ext3, также в отдельные разделы выделены сторонний софт (appfs), настройки программ (configfs), а также файлы пользователя (VFAT).
...
Исходные коды и патчи доступны [здесь](ftp://grids.be/mirror/ftp.ingenic.cn/3sw/01linux/01loader/u-boot/).
...
В файле mbr-uboot-msc.bin есть следующие параметры запуска ядра и самого U-Boot:
````
...
Функция recovery по факту не используется, т.к. этот раздел при прошивке Ritmix RZX-50 оставлен пустым.
...
Загрузочный скрипт расположен в /etc/inittab; из него вызывается скрипт загрузки оболочки (сама оболочка в appfs.ext3).
